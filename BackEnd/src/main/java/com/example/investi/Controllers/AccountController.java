...
public class AccountController {
    ...
    public ResponseEntity<List<Account>> listAllAccountsForAdmin(
            @RequestHeader("Authorization") String token,
            @RequestParam(required = false) String accountType) {

        // Validate admin authorization (you can implement your own logic here)
        if (!isAdmin(token)) {
        ...
        // Fetch accounts based on the filter
        List<Account> accounts = accountService.listAllAccounts(type);
        return ResponseEntity.ok(accounts);
    }

    /**
     * Helper method to validate admin privileges.
     */
    private boolean isAdmin(String token) {
        // Example: Check if the token belongs to an admin user
        return true;
    }

    @PutMapping("/admin/update/{id}")
    public ResponseEntity<Account> updateAccountForAdmin(
        ...
        // Call the service to update the account
        Account account = accountService.updateAccountForAdmin(id, updatedAccount, projectId);
    ...
    public ResponseEntity<Account> getAccountById(
        ...
        // Validate admin or client authorization (you can implement your own logic here)

        // Call the service to fetch the account
